...
def ejecutar_bot_facebook():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Limpiamos y adaptamos los campos para compatibilidad estricta con Playwright
            for cookie in cookie_list:
                if "expirationDate" in cookie:
                    cookie["expires"] = int(cookie.pop("expirationDate"))
                
                # PARCHE CRÍTICO: Playwright no acepta "explicit" en sameSite. Lo cambiamos a "Lax"
                if "sameSite" in cookie:
                    s_site = str(cookie["sameSite"]).capitalize()
                    if s_site in ["Strict", "Lax", "None"]:
                        cookie["sameSite"] = s_site
                    else:
                        cookie["sameSite"] = "Lax"  # Valor seguro por defecto
            ...
            # Inyectamos las cookies corregidas en el navegador emulado
